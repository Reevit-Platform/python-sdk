...
class CustomersService:
    ...
    def get(self, customer_id: str) -> Dict[str, Any]:
        return self.client.request("GET", f"/v1/customers/{_seg(customer_id)}")

    def update(self, customer_id: str, data: Dict[str, Any], idempotency_key: Optional[str] = None) -> Dict[str, Any]:
        headers = {"Idempotency-Key": idempotency_key} if idempotency_key else None
        return self.client.request("PATCH", f"/v1/customers/{_seg(customer_id)}", json=data, headers=headers)

    def delete(self, customer_id: str, idempotency_key: Optional[str] = None) -> None:
        headers = {"Idempotency-Key": idempotency_key} if idempotency_key else None
        self.client.request("DELETE", f"/v1/customers/{_seg(customer_id)}", headers=headers)
    ...
    def payment_history(self, customer_id: str, **params: Any) -> List[Dict[str, Any]]:
        return _extract_list(self.client.request("GET", f"/v1/customers/{_seg(customer_id)}/payments", params=params), "payments")
